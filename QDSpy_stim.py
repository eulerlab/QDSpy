...
class Stim:
  ...
  def defShader(self, _shID, _shType):
    ...
    newShader = [StimObjType.shader, _shID,
                 _shType, self.ShManager.getDefaultParams(_shType)]
  ...
  def defObj_movie(self, _ID, _fName):
    ...
    newMovie = [StimObjType.movie, _ID,
                _fName, tempMovie.nFr, tempMovie.dxFr, tempMovie.dyFr]
  ...
  def compile(self, _Stage):
    ...
    #
    if ((len(self.ObjList) == 0) and (len(self.MovList) == 0) and
        (len(self.VidList) == 0)) or (len(self.SceList) == 0):
    ...
    for sc in self.SceList:
      ...
      ssp.Log.write("ok", "Scene {0} of {1}; {2:.0f}% compiled...".format(
                    sc[SC_field_index], len(self.SceList), percent), True)
...
>>>>>>> refs/remotes/origin/experimental
      ...
      if   sc[SC_field_type] == StimSceType.changeObjCol:
        ...
        for iObj, ID in enumerate(sc[SC_field_IDs]):
          iOb = self.ObjDict[ID]
          ob  = self.ObjList[iOb]
          self.ObjList[iOb][SO_field_fgRGB]\
              = sc[SC_field_RGBs][iObj]
          self.ObjList[iOb][SO_field_alpha]\
              = sc[SC_field_Alphas][iObj]
          self.ObjList[iOb][SO_field_doRGBAByVert]\
              = sc[SC_field_doRGBAByVert][iObj]
      ...
      # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for iObj in range(nObjs):
          ...
          if len(np_vertTr) == len(self.cODr_tr_vertCoord[iLastODr][iObj][2]):
            ...
            #
            if (np_vertTr == self.cODr_tr_vertCoord[iLastODr][iObj][2]).all():
    ...
    # Finished compiling
    #
    ssp.Log.write("ok", "Stimulus '{0}' compiled for {1} Hz refresh"
                  .format(self.nameStr, self.cFreq_Hz))
    self.isComp   = True
    self.LastErrC = StimErrC.ok
