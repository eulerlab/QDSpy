...
class Movie:
  ...
  def __loadMontage(self):
    ...
    # *****************
    # *****************
    if (not(os.path.isfile(self.fNameImg)) or
        not(os.path.isfile(self.fNameDesc))):
    ...
    if (((self.img.width % self.dxFr) > 0) or
        ((self.img.height % self.dyFr) > 0) or
        (n != self.nFr)):
...
class MovieCtrl:
  ...
  def check(self):
    ...
    #
    return ((self.fr0 >= 0) and (self.fr1 >= 0) and
            (self.frreps >= 1) and (self.sqreps >= 1) and
            (self.fr1 < self.nFr))
